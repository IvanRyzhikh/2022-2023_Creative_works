...
// рисует вершину на заданных координатах x и y
void Painter::draw_top(int x, int y, QPainter *painter, QString name) {
    ...
    painter->drawText(QRect(text_x, text_y, radius, text_size), Qt::AlignCenter, name);
}

// рисует ребро между двумя заданными точками, например, (x1, y1) и (x2, y2)
void Painter::draw_edge(int x1, int y1, int x2, int y2, QPainter *painter, int weight, int type) {
    ...
    int l_x1 = (x2 - x1) / line_length * radius + x1;
    int l_y1 = (y2 - y1) / line_length * radius + y1;
    int l_x2 = x2 - (x2 - x1) / line_length * radius;
    int l_y2 = y2 - (y2 - y1) / line_length * radius;
    painter->drawLine(l_x1, l_y1, l_x2, l_y2);
    if(type == 1) {
        double C;
        if (x2==l_x2 && l_y2 < y2) { C = pi * 3 / 2; }
        else if (x2==l_x2 && l_y2 > y2) { C = pi / 2; }
        else if (y2 == l_y2 && l_x2 < x2) { C = pi; }
        else if (y2 == l_y2 && l_x2 < x2) { C = 0; }
        else if (y2 < l_y2 && l_x2 < x2) { C = atan(1.0 * (y2 - l_y2) / (x2 - l_x2)) + pi; }
        else if (y2 > l_y2 && l_x2 < x2) { C = atan(1.0 * (y2 - l_y2) / (x2 - l_x2)) + pi; }
        else if (y2 < l_y2 && l_x2 > x2) { C = atan(1.0 * (y2 - l_y2) / (x2 - l_x2)) + 2 * pi; }
        else if (y2 > l_y2 && l_x2 > x2) { C = atan(1.0 * (y2 - l_y2) / (x2 - l_x2)) + 0; }
    ...
    int text_x = (l_x2 + l_x1) / 2 - radius / 2;
    int text_y = (l_y2 + l_y1) / 2 - text_size / 2;
    painter->drawRect(QRectF(text_x, text_y, radius, text_size));
    painter->drawText(QRect(text_x, text_y, radius, text_size), Qt::AlignCenter, QString::number(weight));
}
